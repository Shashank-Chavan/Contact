...
public class MyDB extends SQLiteOpenHelper {
    ...
    public List<Data> GetContacts(){
        ...
        if(cursor.moveToFirst()){
            do{
                Data data = new Data();
                dataList.add(new Data(cursor.getString(1),cursor.getString(2),cursor.getString(3)));
    ...
    public void Update_Contact(String name,String number,String email,String o_number){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(params.KEY_NAME,name);
        values.put(params.KEY_NUMBER,number);
